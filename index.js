...
$("form").on("submit", (e) => {
  ...
  let validateForm = function () {
    let startN = parseInt(startNode),
      endN = parseInt(endNode),
      numN = parseInt(numOfNodes);
  ...
  if (numOfNodes == 1 && chosenAlgo != "dfs" && chosenAlgo != "bfs") {
    ...
    let runBellmanFord = function () {
      ...
      if (bellmanFord.hasNegativeWeightCycle)
        alert("Negative weight cycle detected. Please try again.");
...
$("#reset").on("click", () => {
  ...
  for (let i = 1; i <= numOfNodes; i++)
    cy.$("#n" + i).removeClass(["highlighted", "shortest-path"]);
  for (let i = 0; i <= edges.length; i++)
    cy.$("#" + edges[i]).removeClass(["highlighted", "shortest-path"]);
  edges = [];
});
